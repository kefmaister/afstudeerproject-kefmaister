    ...
    <div x-data="{ open: false, formAction: '' }">
        ...
        <div x-cloak>
            ...
            <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50">
                <div class="bg-white w-full max-w-md p-6 rounded-lg shadow">
                    <h2 class="text-lg font-bold mb-4">Bedrijf Weigeren</h2>
                    <form :action="formAction" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <input type="text" name="reason" placeholder="Reden voor weigering" required
        ...
        <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
            @if (request()->routeIs('coordinator.inbox.companies'))
                <h2 class="text-lg font-semibold mb-4">In afwachting: Bedrijven</h2>
                @foreach ($pendingCompanies as $company)
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Stad: {{ $company->town }}</p>
                            <p class="text-sm text-gray-600">BTW: {{ $company->company_vat }}</p>
                            <p class="text-sm text-gray-600">E-mail: {{ $company->user->email ?? 'N/A' }}</p>
                        </div>
            ...
            @elseif(request()->routeIs('coordinator.inbox.stages'))
                <h2 class="text-lg font-semibold mb-4">In afwachting: Stages</h2>
                @foreach ($pendingStages as $stage)
                    <div class="bg-gray-50 p-4 rounded shadow mb-4">
                        <p><strong>Titel:</strong> {{ $stage->title }}</p>
                        <p><strong>Taken:</strong> {{ $stage->tasks }}</p>
                        <p><strong>Bedrijf:</strong> {{ $stage->company->company_name ?? '' }}</p>
